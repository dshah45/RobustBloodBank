...
def admin_donor_view(request):
    ...
    mylist = []
    for t in donors:
        mylist.append(t.email)
        print(t.email)
    req = []
    don = []
    for r in requests:
        print(r.request_by_patient)
        req.append("Patient Name:"+str(r.request_by_patient) + "\n" + "Blood Group:" + r.bloodgroup + " \n" + "Unit:"+str(r.unit)+"ml")
        # req.append(r.bloodgroup)
        # req.append(r.unit)
        #don.append(r.request_by_donor)
    lol_string = ' '.join(map(str, req))
    message = f'{lol_string} ml'
        # message += f'\n"{don}"\n\n'
    message += f'\nPlease Visit Any of the Nearby Blood Banks\n\n'
    message += f'\n1. ACTREC BLOOD BANK , Mumbai\n\n'
    message += f'2. KEM HOSPITAL BLOOD BANK, Mumbai\n\n'
    message += f'3. BYTCO HOSPITAL BLOOD BANK, Mumbai\n\n'
    message += f'4. KEM HOSPITAL BLOOD BANK, Mumbai\n\n'
    message += f'5. BYTCO HOSPITAL BLOOD BANK, Mumbai\n\n'
    email_from = settings.EMAIL_HOST_USER

    send_mail('Blood Requirement!!', message, email_from, mylist)
    print("Send")
    ...
    # for r in requests:
    #     print(r.request_by_patient)
    #     req.append(r.request_by_patient)
    #     don.append(r.request_by_donor)
    # message = f'{req} has requested for blood.'
    # message += f'\n"{don}"\n\n'
    # email_from = settings.EMAIL_HOST_USER
    #
    # send_mail('TestMail', message, email_from, ['dshah1359@gmail.com'])
    # print("Send")
    # donors=dmodels.Donor.objects.all()
    # mylist = []
    #
    ...
    #     mylist.append(t.email)
    #     print(t.email)
    # #send_mail('TestMail', 'Kal Clg jana hai??', 'robustblood@gmail.com', ["dshah1359@gmail.com"])
    # print("Send")
    #
...
def admin_request_view(request):
    requests = models.BloodRequest.objects.all().order_by('-updates')
